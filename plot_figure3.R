...
#hist(Ztot,breaks=50,freq=T,main="",ylim=c(0,350),xlim=c(1,1750),xlab="",ylab="",col="white")
...
vecquant=1-(1+vecgamma[26]*((1:10000)+1)/vecsigma[26])^(-1/vecgamma[26])

quantau=27+which(vecquant>=tau)[1]

quantau

############################################

Z=c(11,13,19,10,103,15,12,46,46,12,38,26,42,44,20,32,15,15,40,32,61,24,23,13,14,77,24,17,15,20,732,22,61,74,12,23,16,25,28,29,14,13,16,17,13,21,50,37,10,10,33,12,57,155)

Z=c(Z,rep(2,93),rep(3,4),rep(4,11),rep(5,7),rep(6,2),rep(8,2),rep(9,3))
...
title("SARS-CoV-2 clusters in Hong Kong",line=2.5)
...
abline(a=0.55,b=0,col="red",lty=5)

abline(v=17,col="red",lty=5)
...
uscases=as.numeric(as.vector(uscases[,1]))
...
while(k<=length(vecu)){
  ...
  vecsigmasup[k]=vecsigma[k]+sqrt(varhatsigma)/sqrt(nk)*qnorm(0.95)
  
  vecsigmainf[k]=vecsigma[k]+sqrt(varhatsigma)/sqrt(nk)*qnorm(0.05)
...
plot(vecgammacovid~vecucovid,type="l",xlab="Threshold",ylab="",ylim=c(-1,3),xlim=c(1,100),col="red",axes=F)

title("SARS-CoV-2 clusters in Kerala",line=2.5)

axis(side=2, at = (-2:6)/2,las=1)

axis(side=1, at = (0:20)*5)
...
abline(a=0.36,b=0,col="red",lty=5)
...
title("SARS-CoV-2 clusters in Oregon",line=2.5)
