...
plot(y=meanexcess,x=vecu,pch=1,col="red",ylim=c(0,400),xlim=c(0,400),ylab="Mean excess value",xlab="Threshold (u)",main="SARS-CoV-2 clusters in Canada")
...
hist(Ztot,breaks=20,freq=T,main="",ylim=c(-1,80),xlim=c(1,1500),xlab="",ylab="")

title("SARS-CoV-2 clusters in Canada",line=2.5)

par(new=T)

plot(vecgammacovid~vecucovid,type="l",xlab="Threshold",ylab="Count",col="red",ylim=c(-1,2),xlim=c(0,500),axes=F)

axis(side=4, at = (-2:4)/2)

mtext("xi", side=4, line=-1.5)

axis(side=3, at = (0:10)*50)
...
vecquant=((1+vecgamma[17]*((0:10000))/vecsigma[17])^(-1/vecgamma[17]))*mean(Ztot>=vecu[17])
