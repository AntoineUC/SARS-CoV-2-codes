...
X=read.table(file="sse_korea_2021_period_2.txt",header=T,sep=",")

Y=table(X)

Ztot=Y

Ztot=c(Ztot,rep(0,29193))

x=unique(Ztot)

y=table(factor(Ztot, levels=0:33))

data=as.data.frame(table(factor(Ztot, levels=0:33)))

names(data)=c("Cases","Count")

p=ggplot(data=data, aes(x=Cases, y=log(Count+1))) +
  scale_x_discrete("Cases", breaks=seq(0,33,3))+
  geom_bar(stat="identity", width=0.5)+
  labs(title = "(a) Barplot - Delta data")
...
linreg=lm(meanexcess[1:10]~vecu[1:10])$coefficients
...
p=ggplot(data,aes(x=Threshold, y=Tail_Index)) +
  ...
  labs(title = "(b) Mean excess value plots - Delta data") +
  geom_text(x=4, y=6, label=expression(paste(slope,'=0.280',' => ',xi,'=0.219')) ,col="red")
...
vecuc=seq(min(Ztot),max(Ztot),length=161)
...
p=ggplot(data,aes(x=Threshold, y=Tail_Index)) +
  ...
  ylab("Tail  Index") +
  labs(title = "(c) Three tail index estimates - Delta data")
#+
#  scale_y_continuous(limits=c(-1,1), breaks=seq(-3,3, by=0.1))
...
p=ggplot(data,aes(x=Threshold, y=Tail_Index)) +
  ...
  ylab("Tail  Index") +
  labs(title = "(d) Tail index plot D-GPD - Delta data")
