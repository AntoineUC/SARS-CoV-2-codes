...
mlecov=fitdist(data=Zcov,"nbinom") #negative binomial fit

1/(1+14.068080/4.059985) #approximation of the xi

mlesars=fitdist(data=Zsars,"nbinom") #negative binomial fit

1/(1+36.667694/1.246506) #approximation of the xi

data <- data.frame(
  Disease = c( rep("SARS-CoV-2", 101), rep("SARS-CoV ", 101) ),
  probs = c(dnbinom(0:100,size=4.059985,mu=14.068080),dnbinom(0:100,size=1.246506,mu=36.667694)),
  xaxis = rep(0:100,2)
)
...
print(p+labs(title = "(c) Mean Excess Value plots")
...
print(p+labs(title = "(d) Tail index plots")#+theme_bw()+theme(panel.border = element_blank(), panel.grid.major = element_blank(),
