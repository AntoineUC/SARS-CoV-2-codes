...
plot(y=meanexcess,x=vecu,pch=1,col="red",ylim=c(0,150),xlim=c(0,150),ylab="Mean excess value",xlab="Threshold (u)",main="SARS-CoV-2 clusters in China")

linreg=lm(meanexcess[1:19]~vecu[1:19])$coefficients

abline(a=linreg[1],b=linreg[2],col="red")

linreg[2]/(1+linreg[2])

text(x=100,y=30,labels="a=1.826 => gamma=0.646",col="red")
...
hist(Ztot,breaks=25,freq=T,main="",ylim=c(0,25),xlim=c(1,400),xlab="",ylab="")
...
vecquant=((1+vecgamma[12]*((0:10000))/vecsigma[12])^(-1/vecgamma[12]))*mean(Ztot>=vecu[12])

quantau=16-1+which(vecquant<=1-tau)[1]
