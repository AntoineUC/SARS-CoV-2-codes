...
californiaindex=(latitude<=42)*(latitude>32.30)*(longitude<=-114.08)*(longitude>-124.24)
...
plot(y=meanexcess,x=vecu,pch=1,col="red",ylim=c(0,400),xlim=c(0,400),ylab="Mean excess value",xlab="Threshold (u)",main="SARS-CoV-2 clusters in California")
...
hist(Ztot,breaks=25,freq=T,main="",ylim=c(-1,80),xlim=c(1,3400),xlab="",ylab="",axes=F)

axis(side=4, at = (0:5)*16)

axis(side=3, at = (0:5)*650)

title("SARS-CoV-2 clusters in California",line=2.5)

par(new=T)

plot(vecgammacovid~vecucovid,type="l",xlab="Threshold",ylab="xi",col="red",ylim=c(-1,3),xlim=c(0,1000),axes=F)

axis(side=2, at = (-2:6)/2)

mtext("Count", side=4, line=-1.5)

axis(side=1, at = (0:10)*100)
...
vecquant=1-(1+vecgamma[25]*((1:100000)+1)/vecsigma[25])^(-1/vecgamma[25])
...
sum(country=="United Kingdom")

usindex=c(which(country=="United Kingdom"),which(country=="Ireland"))
...
hist(Ztot,breaks=25,freq=T,main="",ylim=c(0,6),xlim=c(1,300),xlab="",ylab="",axes=F)

axis(side=4, at = (0:3)*2)

axis(side=3, at = (0:5)*60)

title("SARS-CoV-2 clusters in United Kingdom & Ireland",line=2.5)

par(new=T)

plot(vecgammacovid~vecucovid,type="l",xlab="Threshold",ylab="xi",col="red",ylim=c(-1,3),xlim=c(0,200),axes=F)

axis(side=2, at = (-2:6)/2)

mtext("Count", side=4, line=-1.5)

axis(side=1, at = (0:5)*40)

#plot(vecgammacovid~vecucovid,type="l",xlab="Threshold",ylab="gamma",main="SARS-CoV-2 clusters in Great Britain",ylim=c(-1,1),xlim=c(1,150),col="red")
...
vecquant=1-(1+vecgamma[13]*((1:10000)+1)/vecsigma[13])^(-1/vecgamma[13])

quantau=22+which(vecquant>=tau)[1]
