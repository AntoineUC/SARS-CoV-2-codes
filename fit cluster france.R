...
plot(y=meanexcess,x=vecu,pch=1,col="red",ylim=c(0,1200),xlim=c(0,200),ylab="Mean excess value",xlab="Threshold (u)",main="SARS-CoV-2 clusters in France")

linreg=lm(meanexcess[1:21]~vecu[1:21])$coefficients

abline(a=linreg[1],b=linreg[2],col="red")

linreg[2]/(1+linreg[2])

text(x=100,y=30,labels="a=7.148 => gamma=0.877",col="red")
...
hist(Ztot,breaks=25,freq=T,main="",ylim=c(-1,30),xlim=c(1,2500),xlab="",ylab="Count")
...
quantau=26-1+which(vecquant<=1-tau)[1]
